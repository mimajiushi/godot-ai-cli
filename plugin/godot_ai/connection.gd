...
## WebSocket transport to the Godot AI server (in this fork: the godot-ai-cli Go daemon).
...
## dialing the port the server was asked to bind (fork: the Go daemon ws port).
...
## godot-ai version reported by the server in its `handshake_ack` reply (in
## this fork: the godot-ai-cli Go daemon's version; upstream: the Python
## package version). Empty until the authenticated v4 handshake completes.
var server_version := ""

# godot-ai-cli fork patch: daemon 在 handshake_ack 里携带的兼容标记（契约见
# internal/bridge/envelope.go）：plugin_stale=true 表示 daemon 判定本插件与
# 它版本兼容但非精确相等（patch 漂移，如插件 4.1.0 + daemon 4.1.1）——仅作
# 展示/诊断，不影响连接；bundled_plugin_version 是 daemon 捆绑的插件版本。
# 上游 daemon 不带这两个字段（omitempty），保持默认 false/""。
...
func _process(delta: float) -> void:
	...
	match peer_state:
		...
		WebSocketPeer.STATE_CLOSED:
			if _connected:
				_connected = false
				# godot-ai-cli fork patch (RS-021): zero the stale backoff
				# leftover so a drop from OPEN redials on THIS tick.
				# `_attempt_reconnect` seeds the timer with the NEXT retry's
				# delay before dialing; a fast dial leaves most of that delay
				# (up to the 60s cap) parked here for the whole connection.
				# Waiting it out after a drop let kept editors miss the
				# `--upgrade-daemon` reconnect grace.
...
func _clear_on_disconnect() -> void:
	server_version = ""
	# godot-ai-cli fork patch: 软标记同样不得带入下一条连接。
...
func _attempt_reconnect() -> void:
	...
	## a quiet reconnect loop after the server restarts (fork: the Go daemon).
	_peer = WebSocketPeer.new()
...
func _build_auth_response() -> Dictionary:
	...
	# daemon records it as the session origin (stop 保护只动 "cli" 会话)；
	# 该字段同时进入 client_proof 转录，不是可伪造的游离字段。
	var launched_by := _launched_by(OS.get_environment("GODOT_AI_CLI_LAUNCHED"))
	if (
	...
	var client_proof := _client_proof(
		...
		editor_pid,
		launch_mode,
		launched_by, # godot-ai-cli fork patch: 追加进转录末尾
	)
	...
	return {
		...
		# godot-ai-cli fork patch: fork 扩展键（daemon 侧双转录兼容纯上游插件）
		"launched_by": launched_by,
	}


## godot-ai-cli fork patch: pure decision helper for the auth_response's
...
func _handle_handshake_ack(parsed: Dictionary) -> void:
	...
	# godot-ai-cli fork patch: ack 允许额外携带 fork 扩展键 plugin_stale /
	# bundled_plugin_version（fork Go daemon 的软标记，omitempty）；其余键
	# 集合仍须精确匹配。
	var fork_optional_keys: Array[String] = ["plugin_stale", "bundled_plugin_version"]
	if (
		_handshake_complete
		or not _server_verified
		or not _auth_response_sent
		or not _has_exact_keys(parsed, expected_keys, fork_optional_keys)
	...
	# godot-ai-cli fork patch: daemon 侧放宽后的软标记——兼容但 patch 不等时置位。
...
# godot-ai-cli fork patch: optional 参数承载 fork 扩展键（目前仅
# handshake_ack 的 plugin_stale / bundled_plugin_version 使用）。
static func _has_exact_keys(value: Dictionary, expected: Array[String], optional: Array[String] = []) -> bool:
...
static func _client_proof(
	...
	launched_by := "", # godot-ai-cli fork patch: 可选扩展（默认空=上游原样转录）
) -> String:
	# godot-ai-cli fork patch: launched_by 非空时追加到转录末尾；
	# daemon 侧先试扩展转录、回退上游转录，纯上游插件因此仍可连接。
	...
	if not launched_by.is_empty():
		transcript.append(launched_by)
	return _hmac_proof(
		capability,
