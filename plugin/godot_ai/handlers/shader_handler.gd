...
func create_shader(params: Dictionary) -> Dictionary:
	...
	var size_err: Variant = _size_error(code)
	if size_err != null:
		return size_err
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var type_err: Variant = _shader_type_error(shader_type)
	if type_err != null:
		return type_err
	...
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var write_err: Variant = _write_atomic(resource_path, code, kind)
	if write_err != null:
		return write_err
...
func patch_shader(params: Dictionary) -> Dictionary:
	...
	if replace_all:
		...
		replacements = 1
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var size_err: Variant = _size_error(new_content)
	if size_err != null:
		return size_err
	...
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var write_err: Variant = _write_atomic(path, new_content, kind)
	if write_err != null:
		return write_err
