...
func _edit_add_node(
	...
	if raw_id == null:
		...
		var canonical: Variant = _canonical_id(raw_id)
...
func _edit_connection(
	...
	if target.has("error"):
		return target
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var from_port: Variant = _integral_number(operation.get("from_port"))
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var to_port: Variant = _integral_number(operation.get("to_port"))
...
func _edit_node_id(shader: VisualShader, stage: int, raw: Variant, aliases: Dictionary) -> Dictionary:
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var canonical: Variant = _canonical_id(raw)
	...
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var number: Variant = _integral_number(canonical)
	if number == null:
...
func _build_stage(shader: VisualShader, spec: Dictionary) -> Dictionary:
	...
	for edge in spec.connections:
		...
		# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
		var from_port_value: Variant = _integral_number(edge.from_port)
		# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
		var to_port_value: Variant = _integral_number(edge.to_port)
...
func _parse_varying(entry: Dictionary, _label: String) -> Dictionary:
	...
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var mode: Variant = _enum_lookup(VARYING_MODE_VALUES, entry.get("mode"), "varying_mode_")
	if mode == null:
		return _invalid("varying mode must be one of: %s" % ", ".join(VARYING_MODE_VALUES.keys()))
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var varying_type: Variant = _enum_lookup(VARYING_TYPE_VALUES, entry.get("type"), "varying_type_")
...
static func _canonical_id(value: Variant) -> Variant:
	if value is String:
		return value
	# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
	var number: Variant = _integral_number(value)
...
static func _enum_value(hint: String, value: Variant) -> Dictionary:
	...
	for item in hint.split(","):
		...
		if parts.size() > 1:
			index = int(parts[1])
		# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
		var numeric: Variant = _integral_number(value)
...
static func _typed_value(type: int, value: Variant) -> Dictionary:
	match type:
		...
		TYPE_INT:
			# godot-ai-cli fork patch: Variant 显式标注（demo 工程把 INFERRED_DECLARATION 视为错误）。
			var integer: Variant = _integral_number(value)
