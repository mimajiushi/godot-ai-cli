...
func _build_ui() -> void:
	...
	## ---- godot-ai-cli fork patch: self-update removed -----------------
	## The fork never creates the UpdateManager: plugin updates ship with
	## the godot-ai-cli release instead of an in-editor self-updater. The
	## banner above stays hidden (`visible = false`) and every other
	...
	## CLI/skill, not the dock — hide the entry points.
	...
	## godot-ai-cli fork patch: hidden (see above).
	...
	# --- Dev mode toggle (always visible upstream; hidden in the fork) ---
	...
	## godot-ai-cli fork patch: dev mode only exposes Python dev-server
	## controls, which the fork disables — hide the toggle entirely.
	dev_toggle_row.visible = not ForkConfig.external_daemon_mode()
...
func _apply_dev_mode_visibility() -> void:
	...
	## godot-ai-cli fork patch: the Setup section only offers uv/dev-server
	## controls, both meaningless without a Python backend — never show it.
	_setup_section.visible = (
		_setup_section_should_show(dev, uv_missing, _server_launch_pending())
		and not ForkConfig.external_daemon_mode()
	)
...
func _refresh_drift_banner(mismatched_ids: Array[String]) -> void:
	if _drift_banner == null:
		return
	## godot-ai-cli fork patch: the drift banner's only action is
	## reconfiguring MCP clients, which the fork leaves to the CLI/skill —
