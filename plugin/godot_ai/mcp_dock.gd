...
# godot-ai-cli fork patch: fork 的功能开关（UI 门控集中到这里判定）。
...
func _build_ui() -> void:
	...
	# godot-ai-cli fork patch: MCP client configuration is handled by the
	# CLI/skill in the fork — the refresh sweep button stays hidden.
	if ForkConfig.mcp_client_config_disabled():
		clients_refresh_btn.visible = false
	...
	# godot-ai-cli fork patch: hidden（同上——窗口内容全是 client 配置）。
	if ForkConfig.mcp_client_config_disabled():
		clients_open_btn.visible = false
	...
	# godot-ai-cli fork patch: the CTA opens client configuration, which the
	# fork 关闭——永远隐藏（下游 visible=true 的赋值点见 _refresh_clients 段，
	# 那里已随巡检门控不再触发；这里再强制一次兜底）。
	if ForkConfig.mcp_client_config_disabled():
		_client_empty_cta_btn.visible = false
		_client_empty_cta_btn.hide()
	...
	# godot-ai-cli fork patch: dev mode 只暴露 Python dev-server / uv
	# 控制，在 fork 里无意义——整行隐藏。
	if ForkConfig.external_daemon_mode():
		dev_toggle_row.visible = false
...
func _apply_dev_mode_visibility() -> void:
	...
	# godot-ai-cli fork patch: Setup 区在 fork 里恒隐藏（uv/dev-server 控制
	# 无意义），不走下面的上游判定。
	if ForkConfig.external_daemon_mode():
		_setup_section.visible = false
		return
...
func _refresh_server_version_label(server_status: Dictionary = {}) -> void:
	...
	if _server_restart_in_progress:
		...
		# godot-ai-cli fork patch: 兼容但 patch 漂移（如插件 4.1.0 + daemon
		# 4.1.1）时以软警告文案替代裸的 expected 提示——允许连接，只提示对齐版本。
		if not is_incompatible and McpVersionCompat.versions_compatible(server_ver, expected_ver):
			var note := McpVersionCompat.compatible_mismatch_note(server_ver, expected_ver)
...
func _refresh_setup_status() -> void:
	if _setup_container == null:
		return
	# godot-ai-cli fork patch: Setup 区只提供 uv/dev-server 控制，在无
	# Python 后端的 fork 里无意义——整区隐藏。
	if ForkConfig.external_daemon_mode():
		_setup_section.visible = false
		return
...
func _dispatch_client_action(client_id: String, action: String) -> void:
	# godot-ai-cli fork patch: defense in depth —— Configure/Remove 会在
	# worker 线程 spawn 子进程；fork 里 client 配置关闭，动作不派发。
	if ForkConfig.mcp_client_config_disabled():
		return
	if _is_self_update_in_progress():
		return
...
func _on_open_clients_window() -> void:
	# godot-ai-cli fork patch: 窗口内容全是 MCP client 配置——fork 里
	# 一律不开（按钮已隐藏，这里是纵深防御）。
	if ForkConfig.mcp_client_config_disabled():
		return
...
func _refresh_clients_summary() -> void:
	...
	if _client_empty_cta_btn != null:
		# godot-ai-cli fork patch: CTA 在 fork 里恒隐藏（client 配置关闭）。
		_client_empty_cta_btn.visible = (
			false
			if ForkConfig.mcp_client_config_disabled()
			else configured == 0 and _client_status_refresh_has_completed()
		)
...
func _request_client_status_refresh(force: bool = false) -> bool:
	# godot-ai-cli fork patch: 巡检会为每个客户端 spawn 子进程探针——
	# fork 关闭 client 配置，巡检请求直接拒绝（worker 侧另有门控）。
	if ForkConfig.mcp_client_config_disabled():
		return false
...
func _refresh_drift_banner(mismatched_ids: Array[String]) -> void:
	if _drift_banner == null:
		return
	# godot-ai-cli fork patch: drift banner 的唯一动作是 reconfigure
	#（client 配置）——fork 里恒隐藏。
	if ForkConfig.mcp_client_config_disabled():
		_drift_banner.visible = false
		return
...
func present_update_check(result: Dictionary) -> void:
	# godot-ai-cli fork patch: 自更新器在 fork 里不存在（更新随
	# godot-ai-cli 发行），更新横幅恒隐藏——纵深防御，正常路径下
	# _update_manager 为 null，本函数根本不会被调用。
	if ForkConfig.v4_self_update_disabled():
		return
...
func present_update_state(state: Dictionary) -> void:
	...
	if state.has("banner_visible") and _update_banner != null:
		# godot-ai-cli fork patch: 自更新横幅在 fork 里恒隐藏。
		_update_banner.visible = false if ForkConfig.v4_self_update_disabled() else bool(state["banner_visible"])
	if state.has("label_text") and _update_label != null:
