...
func _enter_tree() -> void:
	...
	## off below, the client handler would be declared but unreachable — the
	## dispatcher's lazy-surface audit requires every declared handler to back
	## at least one command, so skip the declaration too.
...
func force_restart_or_start_dev_server() -> bool:
	## godot-ai-cli fork patch: dev-server spawning (and the port-owner
	## kill that precedes it) is disabled — the Go daemon owns the ports
	## and must never be killed or replaced by the plugin.
	if ForkConfig.external_daemon_mode():
		print("MCP | godot-ai-cli fork: dev server is managed by godot-ai-cli, not by the editor plugin")
		return false
...
func start_dev_server() -> void:
	## godot-ai-cli fork patch: never spawn the Python dev server (the Go
	## daemon is the only backend). Gated call keeps the dormant upstream
	## body below compiling without unreachable-code warnings.
	if ForkConfig.external_daemon_mode():
		print("MCP | godot-ai-cli fork: start_dev_server is a no-op — use `godot-ai-cli launch` instead")
		return
