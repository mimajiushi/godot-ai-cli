...
# godot-ai-cli fork patch: fork 的功能开关集中处（external_daemon_mode /
# mcp_client_config_disabled 恒 true）。
...
func _continue_enter_tree_after_update_barrier() -> void:
	...
	# godot-ai-cli fork patch: self-update removed —— fork 的插件更新随
	# godot-ai-cli 发行（plugin install），不构建 v4 自更新器；更新横幅
	# 保持隐藏，其余 `_update_manager` 引用上游均已 null 守卫。
	if not ForkConfig.v4_self_update_disabled():
	...
	# godot-ai-cli fork patch: with the client-config wire commands guarded
	# off (below), declaring the "client" handler would trip the
	# dispatcher's lazy-surface audit — skip the declaration too.
	if not ForkConfig.mcp_client_config_disabled():
	...
	# godot-ai-cli fork patch: tileset handler 需 _connection（物理层写盘时 pause_processing 守卫）。
	...
	# godot-ai-cli fork patch: SpriteFrames 处理器是 fork 独有（非上游）。
	...
	# godot-ai-cli fork patch: continue 停在 debugger break 的游戏 /
	# 把游戏窗口拉到前台（fork 独有命令，CLI `project continue|focus`）。
	...
	# registered in the fork（client 配置是 CLI/skill 的职责）；with no
	# command backing the "client" handler, it must not be declared either —
	# the dispatcher's lazy-surface audit requires every declared handler to
	# back at least one command. gated on ForkConfig.mcp_client_config_disabled().
	if not ForkConfig.mcp_client_config_disabled():
	...
	# godot-ai-cli fork patch: fork 独有命令注册（SpriteFrames 批次/逐帧、
	# TileSet 物理层）——对应 CLI 的 resource spriteframes-* 与
	# tileset add-physics-layer / set-tile-collision。
...
func _on_dock_dev_server_action_requested(action: int) -> void:
	# godot-ai-cli fork patch: dev-server 操作只暴露 Python dev-server /
	# uv 控制，在无 Python 后端的 fork 里无意义（Dock 侧 UI 已隐藏，
	# 这里是纵深防御）。Godot daemon 的生命周期由 CLI 拥有。
...
func _release_normal_startup() -> void:
	...
	# godot-ai-cli fork patch: 自更新器不存在时不触发检查（fork 补丁）。
