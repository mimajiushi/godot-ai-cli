...
# godot-ai-cli fork patch: fork 的功能开关（client 配置在 fork 里由
# CLI/skill 负责，编辑器侧一律不跑）。
...
func request_status_refresh(ids: Array[String], force := false) -> bool:
	# godot-ai-cli fork patch: MCP client 状态巡检会为每个客户端 spawn
	# 有界子进程探针——fork 里 client 配置整体关闭，巡检直接跳过
	# （Dock 侧入口已隐藏，这里是纵深防御）。
