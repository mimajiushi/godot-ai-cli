...
const READY := 3
# godot-ai-cli fork patch（D1 自愈的 v4 移植说明）：上游 v4 删除了旧
# can_transition 状态表，旧 fork 的 "INCOMPATIBLE→READY 自愈出口" 改由
# server_lifecycle.gd 的 BLOCKED 退避重探机制承载（BLOCKED_RECHECK_*）。
