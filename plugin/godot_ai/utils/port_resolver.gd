...
static func commandline_is_godot_ai_server(commandline: String) -> bool:
	...
	if branded and (lower.contains("--pid-file") or lower.contains("--transport")):
		return true
	## godot-ai-cli fork patch: Go daemon 的等价品牌。`godot-ai-cli serve
	## --http-port N --ws-port M` 不带 Python 服务器的 --pid-file/--transport
	## 身份 flag，不做等价认定的话插件自己 spawn 的 daemon 永远过不了
	## require_brand 的 kill-grant 取证（RS-016 步骤 5，beta.24 v4.1.0 重放
	## 抓获：unbranded → launch_unproven）。"二进制路径含品牌 + serve 子命令
	## + --http-port"的组合足够特异，不会误杀无关进程。
	return branded and lower.contains(" serve ") and lower.contains("--http-port")
