...
# godot-ai-cli fork patch: fork 的功能开关与 Go daemon 定位器。
...
# godot-ai-cli fork patch（D1 自愈的 v4 移植）：BLOCKED 不再是只能手动
# Replace/Restart 的终态——按退避节奏自动重探端口，占用者一旦换成兼容
# daemon（或原判定系误判）即收养并转 READY，连接经 transport_ready →
# authorize_transport 自动解锁。前三次 5/15/30s 摸底，之后稳态 60s 一次，
# 代价为每分钟最多一次本地 HTTP 探针。
# （上游 v4 删除了旧 McpServerState.can_transition 状态表，旧 D1 的
# INCOMPATIBLE→READY 出口在此以重探机制重述。）
# beta.25 扩员（runner-3 慢替换活体抓获）：free→LAUNCH 失败的
# no_command / launch_failed / launch_unproven / capability_dir_unwritable
# 也布防重探——fork 插件默认环境 spawn 不出 Go daemon（回落路径拿不到
# server_command），编辑器此前永久卡 BLOCKED，之后手动/CLI 拉起的兼容
# daemon 永远不会被收养；重探对空闲端口会再试 launch（代价同上界），
# 对占线端口则转 occupied/compatible 正常收养。
const BLOCKED_RECHECK_DELAYS_SECONDS: Array[float] = [5.0, 15.0, 30.0]
const BLOCKED_RECHECK_STEADY_SECONDS := 60.0
const BLOCKED_RECHECK_REASONS: Array[String] = [
	"incompatible",
	"occupied",
	"no_command",
	"launch_failed",
	"launch_unproven",
	"capability_dir_unwritable",
]
...
# godot-ai-cli fork patch（D1 自愈）：BLOCKED 重探的退避计数与待触发的 episode id。
var _blocked_recheck_attempts := 0
var _blocked_recheck_pending_episode := 0
# godot-ai-cli fork patch: 兼容但 patch 漂移（4.1.0 ↔ 4.1.1）时的软警告文案，
# READY 状态下由 get_status_dict() 透出（version_note，dock 琥珀色展示）；
# 硬阻断 / 精确相等时为空。
var _server_version_note := ""
...
func transport_authenticated(version: String) -> void:
	...
	if _server_version_compatibility(version, expected).get("compatible", false):
		...
		# godot-ai-cli fork patch: patch 级漂移（4.1.0 ↔ 4.1.1）允许连接，
		# 仅记录软警告（dock 琥珀色展示），不再硬阻断。
		_server_version_note = McpVersionCompat.compatible_mismatch_note(version, expected)
		if not _server_version_note.is_empty():
			push_warning("MCP | " + _server_version_note)
		_publish()
		return
	var public_transport: Dictionary = _transport.public_snapshot() if _transport != null else {}
	# godot-ai-cli fork patch: minor/major 错配的硬阻断补版本对齐指引。
	_block(
		"incompatible",
		"Server v%s does not match plugin v%s. Choose Replace to continue. To align versions: %s"
		% [version, expected, McpVersionCompat.incompatible_align_hint()],
		{
...
func _recheck_blocked_after(delay_seconds: float, episode_id: int) -> void:
	var tree := Engine.get_main_loop()
	if tree is SceneTree:
		await (tree as SceneTree).create_timer(delay_seconds).timeout
	if not is_instance_valid(self):
		return
	recover_blocked_episode(episode_id)


# godot-ai-cli fork patch（D1 自愈）：定时重探入口。与 recover_lost_endpoint
# 同一接缝——重新走完整的 probe→adopt/launch 链，兼容性判定与取证只存在于
# _effect_probe 一处；episode 被取代或状态/原因已变化时定时器自动失效。
func recover_blocked_episode(episode_id: int) -> bool:
	if episode_id <= 0 or episode_id != _blocked_recheck_pending_episode:
		return false
	if episode_id != int(_episode.get("id", -1)):
		_blocked_recheck_pending_episode = 0
		return false
	if (
		str(_episode.get("state")) != BLOCKED
		or str(_episode.get("reason")) not in BLOCKED_RECHECK_REASONS
	):
		_blocked_recheck_pending_episode = 0
		return false
	_blocked_recheck_pending_episode = 0
...
# godot-ai-cli fork patch（D1 自愈）：为可自愈的 BLOCKED 原因安排下一次重探。
# 不可自愈的原因（endpoint_lost 有独立机制、其余系操作/权限类）复位计数。
func _arm_blocked_recheck(reason: String) -> void:
	_blocked_recheck_pending_episode = 0
	if reason not in BLOCKED_RECHECK_REASONS:
		_blocked_recheck_attempts = 0
		return
	...
	if episode_id <= 0:
		return
	var delay := BLOCKED_RECHECK_STEADY_SECONDS
	if _blocked_recheck_attempts < BLOCKED_RECHECK_DELAYS_SECONDS.size():
		delay = BLOCKED_RECHECK_DELAYS_SECONDS[_blocked_recheck_attempts]
	_blocked_recheck_attempts += 1
	_blocked_recheck_pending_episode = episode_id
	_recheck_blocked_after(delay, episode_id)
...
func _ready(kind: String, transport, version: String) -> void:
	...
	# godot-ai-cli fork patch（D1 自愈）：自愈成功/正常就绪后复位重探退避。
	_blocked_recheck_attempts = 0
	_blocked_recheck_pending_episode = 0
	# godot-ai-cli fork patch: patch 级漂移的 adopt/spawn（如插件 4.1.0 +
	# daemon 4.1.1）——兼容，记录软警告并继续，不再走 INCOMPATIBLE 硬阻断。
	_server_version_note = McpVersionCompat.compatible_mismatch_note(
		version, str(_episode.get("expected_version", _plan.get("expected_version", "")))
	)
	if not _server_version_note.is_empty():
		push_warning("MCP | " + _server_version_note)
...
func _block(reason: String, message: String, target: Dictionary = {}) -> void:
	...
	_episode["effect"] = ""
	# godot-ai-cli fork patch: 硬阻断落地时清掉软警告——dock 只应展示其中一种。
	if reason == "incompatible":
		_server_version_note = ""
	_publish()
	transport_cleared.emit(message)
	startup_finished.emit("blocked:%s" % reason)
	# godot-ai-cli fork patch（D1 自愈）：incompatible/occupied 武装退避重探。
	_arm_blocked_recheck(reason)
...
func _effect_probe(payload: Dictionary) -> Dictionary:
	...
	if _authenticated_status_matches_record(live, capability):
		...
		var incompatible := _blocked_probe_result("incompatible", port, live, true)
		# godot-ai-cli fork patch: minor/major 错配的硬阻断补版本对齐指引；
...
func _effect_launch(payload: Dictionary) -> Dictionary:
	...
	# godot-ai-cli fork patch: godot-ai-cli daemon first —— 找到 CLI 二进制
	# 且其捆绑插件版本与本插件精确相等时，spawn Go daemon 取代上游
	# Python 服务器（Python spawn 会以 FOREIGN_SERVER 挡住后续 CLI
	# launch；plugin-spawned daemon 能被 EnsureRunning 透明採用）。
	# capability 对由 spawn_capability_process 统一注入环境，Go daemon
	# 从 GODOT_AI_HTTP_CAPABILITY / GODOT_AI_WS_TOKEN 读取。返回空
	# Dictionary 表示回落到下面的上游 Python spawn（原样保留）。
	var cli_launch := _effect_launch_cli_daemon(payload, port)
	if not cli_launch.is_empty():
		return cli_launch
...
## godot-ai-cli fork patch: 代写 launch 的 pid_file（static + pure，套件可
## 直接钉）。返回是否写入成功；pid_file 为空或 pid 非法时按未写入处理。
static func _publish_launch_pid_file(pid_file: String, pid: int) -> bool:
	if pid_file.is_empty() or pid <= 1:
		return false
	...
	f.close()
	return PortResolver.read_pid_file(pid_file) == pid


## godot-ai-cli fork patch: spawn godot-ai-cli 的 Go daemon 取代上游
## Python 服务器。返回空 Dictionary 表示"本路径不适用"（未开外部
## daemon 模式 / 找不到二进制 / 版本守卫拒绝），调用方回落上游 spawn。
##
## 与上游 _effect_launch 正文的差异（全部因为 daemon 不是 Python
## 服务器）：
##   - 不暂存 Python 专有环境（GODOT_AI_OWNER_PID / GODOT_AI_NO_IDLE_EXIT /
##     GODOT_AI_WAIT_FOR_PORT_MS / PYTHONPATH / GODOT_AI_DISABLE_TELEMETRY）；
##   - pid_file / startup_report 是 Python 后端的产物，Go daemon 不写——
##     pid_file 改由本路径预清并代写（PROVE/stop/replace 全依赖它），
##     startup_report 保持不预清；
##   - capability 对仍由 spawn_capability_process 注入环境变量，daemon
##     启动后用它发布能力记录（internal/capability），prove 环节照常读
##     记录 + Bearer 探针完成认证採用。
func _effect_launch_cli_daemon(payload: Dictionary, port: int) -> Dictionary:
	if not ForkConfig.external_daemon_mode():
		return {}
	var bin := CliDaemon.find_bin()
	if bin.is_empty():
		return {}
	## spawn 守卫（beta.13 事故回归）：PATH 里的 CLI 可能捆绑了不同版本
	## 的插件代码；`<bin> -v` 解析出的 bundled plugin 版本与本插件精确
	## 相等才允许 spawn。本函数运行在 effect worker 线程上，子进程调用
	## 合法。
	if not CliDaemon.should_spawn(
		CliDaemon.probe_version_output(bin),
	...
	if not directory_problem.is_empty():
		return {"ok": false, "reason": "capability_dir_unwritable", "message": directory_problem}
	## godot-ai-cli fork patch: pid_file 预清（与上游 _effect_launch 一致）。
	## 下方 spawn 成功后由插件代写该文件；残留的旧 pid 会在写入前的窗口里
	## 被 PROVE 当成这次 launch 的（RS-016 步骤 5 复验抓获）。
	var pid_file := str(payload.get("pid_file", ""))
	if not pid_file.is_empty() and FileAccess.file_exists(pid_file):
		DirAccess.remove_absolute(pid_file)
	var args: Array[String] = [
		"serve", "--http-port", str(port), "--ws-port", str(int(payload.ws_port)),
	]
	var spawned := spawn_capability_process(bin, args, {})
	var pid := int(spawned.pid)
	if pid <= 1:
		return {"ok": false, "reason": "launch_failed", "message": "The godot-ai-cli daemon process could not be launched."}
	## godot-ai-cli fork patch: Go daemon 不写 pid_file（那是 Python 后端的
	## 产物），由插件代写——PROVE 的 pid 读取（_effect_prove 的 pid_file 闸）
	## 与 stop/replace 的 server_pid 来源全都依赖这个文件。写入的是我们自己
	## spawn 出来的 pid，可信；写失败不阻断（PROVE 会以 pending 重试到超时，
	## 与上游"服务器还没来得及写"同语义）。
	_publish_launch_pid_file(pid_file, pid)
	## 与上游相同的进程身份取证（kill grant），替换/停止路径依赖它。
	var capture_started := Time.get_ticks_msec()
	var attempts: Array = []
...
func get_status_dict() -> Dictionary:
	...
	return {
		...
		# godot-ai-cli fork patch: patch 漂移软警告（dock 琥珀色展示用）。
