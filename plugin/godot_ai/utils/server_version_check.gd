...
static func evaluate(actual_version: String, expected_version: String) -> Dictionary:
	...
	# godot-ai-cli fork patch: 上游这里要求严格相等；fork 放宽为
	# major.minor 兼容（patch 可双向漂移，dev/预发布后缀仍拒绝），
	# 与 daemon 侧 internal/pluginmeta.Compatible 互为镜像。
	# 判定口径单一来源：utils/version_compat.gd（McpVersionCompat）。
	var c := McpVersionCompat.compatibility(actual_version, expected_version)
	return {
		"compatible": bool(c.get("compatible", false)),
		"reason": "" if bool(c.get("compatible", false)) else "version_mismatch",
	}
